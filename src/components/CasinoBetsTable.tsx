...
import type { Bet } from "./BetModal";

const dummyBets = [
  { game: "Kocke", user: "Skrito", time: "12:41", betAmount: 25, multiplier: 2.1, payout: 52.5 },
  { game: "Ruleta", user: "Alice", time: "12:42", betAmount: 10, multiplier: 5.0, payout: 50 },
  { game: "Metanje kovanca", user: "Bob", time: "12:43", betAmount: 15, multiplier: 1.9, payout: 28.5 },
  { game: "Igralni avtomati", user: "Skrito", time: "12:44", betAmount: 5, multiplier: 10, payout: 50 },
  { game: "Kocke", user: "Charlie", time: "12:45", betAmount: 40, multiplier: 1.5, payout: 60 },
  { game: "Ruleta", user: "Skrito", time: "12:46", betAmount: 20, multiplier: 3.2, payout: 64 },
  { game: "Metanje kovanca", user: "Alice", time: "12:47", betAmount: 12, multiplier: 2.0, payout: 24 },
];

const CasinoBetsTable: React.FC = () => {
  const [selectedBet, setSelectedBet] = useState<Bet | null>(null);

  const betsWithIds: Bet[] = useMemo(() => {
    const pad = (n: number) => n.toString().padStart(3, "0");
    return dummyBets.map((b, i) => ({
      ...b,
      id: `432,136,420,${pad(500 + i)}`,
      placedAt: `dne 21.12.2025 ob ${b.time}`,
    }));
  }, []);
    ...
    <>
      <div className="max-w-full w-[1100px] bg-[#0d1c28] rounded-xl p-4 text-base text-slate-200 font-medium">
        <div className="grid grid-cols-6 gap-4 text-slate-400 mb-2 text-sm tracking-wide">
          <span className="text-left">Igra</span>
          <span className="text-left">Uporabnik</span>
          <span className="text-right">Čas</span>
          <span className="text-right">Stava</span>
          <span className="text-right">Množitelj</span>
          <span className="text-right">Izplačilo</span>
        </div>
        <div className="space-y-2">
          {betsWithIds.map((bet, index) => (
            <div
              key={bet.id}
              onClick={() => setSelectedBet(bet)}
              className={`grid grid-cols-6 gap-4 p-3 rounded-lg text-base cursor-pointer transition ${
                index % 2 === 0
                  ? "bg-slate-700/60 hover:bg-slate-700/80"
                  : "bg-slate-800 hover:bg-slate-700/40"
              }`}
            >
              <span className="flex items-center space-x-2 group">
                <span className="w-5 h-5 flex-shrink-0 text-slate-400 transition-colors group-hover:text-white">
                  ...
                  {bet.game === "Igralni avtomati" && (
                    <svg viewBox="0 0 24 24" fill="currentColor">
                      <text x="2" y="18" fontSize="14" fontWeight="900" stroke="currentColor" strokeWidth="2">
                        7
                      </text>
                      <text x="9" y="18" fontSize="15" fontWeight="900" stroke="currentColor" strokeWidth="2">
                        7
                      </text>
                      <text x="16" y="18" fontSize="14" fontWeight="900" stroke="currentColor">
                        7
                      </text>
                    </svg>
                  )}
                  {bet.game === "Metanje kovanca" && (
                    <svg viewBox="0 0 24 24" fill="currentColor">
              ...
              <span className="text-left relative group">
                {bet.user}
                {bet.user === "Skrito" && (
      ...
      {selectedBet && <BetModal bet={selectedBet} onClose={() => setSelectedBet(null)} />}
    </>
  );
};
